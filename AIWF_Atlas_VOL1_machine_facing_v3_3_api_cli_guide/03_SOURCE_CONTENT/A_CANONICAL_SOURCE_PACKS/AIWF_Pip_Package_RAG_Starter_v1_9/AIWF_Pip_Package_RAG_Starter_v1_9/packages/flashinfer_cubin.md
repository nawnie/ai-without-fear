...
## Identity
- **pip name:** `flashinfer-cubin`
- **import/module clue:** `flashinfer`
- **category:** `llm_attention_kernel_binary_cache`
- **AIWF confidence:** source-reviewed for v1.6 LLM serving and OpenAI-compatible local server support.

## What it is
Optional FlashInfer package providing precompiled kernel binaries to reduce first-run compile/download overhead.
...
```powershell
python -m pip show flashinfer-cubin
python - <<'PY'
import importlib
mod = 'flashinfer'
try:
