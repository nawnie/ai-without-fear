...
## Identity
- **pip name:** `nvidia-pyindex`
- **import/module clue:** `nvidia_pyindex`
- **category:** `nvidia_package_index`
- **AIWF confidence:** source-reviewed for role and install-risk classification.

## What it is
NVIDIA NGC/PyPI index helper; older official CUDA pip-wheel docs use it to configure access to NVIDIA-hosted packages.
...
Prefer explicit --extra-index-url in reproducible requirements; record which index was used.
...
1. Confirm the package is actually installed in the active venv: `python -m pip show nvidia-pyindex`.
...
## Windows risk
**low**. The main risk is not the package name by itself; it is mixing CUDA component families, system Toolkit versions, PyTorch-bundled libraries, and custom node source builds without recording the full environment.
