...
- **import/module clue:** `flashinfer`
- **category:** `llm_attention_kernel_library`
- **AIWF confidence:** source-reviewed for v1.6 LLM serving and OpenAI-compatible local server support.

## What it is
Kernel library for LLM serving; package name is flashinfer-python, not flashinfer. Used around advanced vLLM/SGLang attention paths.
...
```powershell
python -m pip show flashinfer-python
python - <<'PY'
import importlib
mod = 'flashinfer'
try:
...
## Sources
https://docs.flashinfer.ai/installation.html | https://docs.vllm.ai/en/latest/features/quantization/
