...
## Identity
- **pip name:** `nvidia-cudnn`
- **import/module clue:** `nvidia.cudnn / meta package`
- **category:** `nvidia_cudnn_meta_package`
- **AIWF confidence:** source-reviewed for role, install-risk classification, and cuDNN/CUDA stack placement.

## What it is
NVIDIA cuDNN meta/package-index name used around cuDNN component distribution.
...
Prefer explicit CUDA-family packages such as nvidia-cudnn-cu12 or nvidia-cudnn-cu13 for reproducible wheelhouses. Meta names are useful for discovery but weaker for support manifests.
...
1. `python -m pip show nvidia-cudnn` if this is an installed package path.
...
## AIWF install stance
Record explicit concrete packages in lockfiles. Use meta/package-index names only as a research cue unless official docs for the target runtime request them.
