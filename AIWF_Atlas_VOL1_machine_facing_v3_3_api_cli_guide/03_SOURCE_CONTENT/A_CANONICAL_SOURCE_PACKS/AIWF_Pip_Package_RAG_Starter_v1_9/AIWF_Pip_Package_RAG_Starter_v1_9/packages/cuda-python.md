...
## Identity
- **pip name:** `cuda-python`
- **import/module clue:** `cuda`
- **category:** `nvidia_cuda_python_api`
- **AIWF confidence:** source-reviewed for role, install-risk classification, and cuDNN/CUDA stack placement.

## What it is
NVIDIA CUDA Python meta-package ecosystem for accessing CUDA platform APIs from Python.
...
Useful for diagnostics, CUDA component discovery, and advanced Python GPU tooling. It is not a replacement for PyTorch and does not by itself fix ComfyUI CUDA issues.
...
1. `python -m pip show cuda-python` if this is an installed package path.
...
## AIWF install stance
Useful in support tooling because CUDA Python includes pieces such as cuda.bindings and cuda.pathfinder. Keep it separated from PyTorch install advice.
