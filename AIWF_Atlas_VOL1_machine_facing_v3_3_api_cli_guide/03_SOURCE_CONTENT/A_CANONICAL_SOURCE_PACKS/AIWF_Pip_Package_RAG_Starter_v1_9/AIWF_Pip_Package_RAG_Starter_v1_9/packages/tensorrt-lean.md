...
## Identity
- **pip name:** `tensorrt-lean`
- **import/module clue:** `tensorrt lean runtime`
- **category:** `nvidia_tensorrt_lean_runtime`
- **AIWF confidence:** source-reviewed for TensorRT/NVIDIA inference stack placement and support-risk classification.

## What it is
TensorRT lean runtime family for smaller deployment/runtime footprints where full build-time capability is not required.
...
Lean runtime is not the default for build/debug workflows. Use only when deployment size/runtime-only constraints are understood.
...
1. `python -m pip show tensorrt-lean`.
