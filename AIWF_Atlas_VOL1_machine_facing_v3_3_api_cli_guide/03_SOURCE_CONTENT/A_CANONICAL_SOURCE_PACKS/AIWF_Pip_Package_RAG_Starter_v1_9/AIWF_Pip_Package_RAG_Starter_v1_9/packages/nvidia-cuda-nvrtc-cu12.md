...
## Identity
- **pip name:** `nvidia-cuda-nvrtc-cu12`
- **import/module clue:** `nvidia.cuda_nvrtc`
- **category:** `nvidia_cuda_runtime_compiler`
- **AIWF confidence:** source-reviewed for role and install-risk classification.

## What it is
NVRTC runtime compiler package for CUDA 12-family Python environments; used by systems that compile kernels at runtime.
...
Runtime/JIT compiler package; not a substitute for full nvcc source-build workflow.
...
1. Confirm the package is actually installed in the active venv: `python -m pip show nvidia-cuda-nvrtc-cu12`.
