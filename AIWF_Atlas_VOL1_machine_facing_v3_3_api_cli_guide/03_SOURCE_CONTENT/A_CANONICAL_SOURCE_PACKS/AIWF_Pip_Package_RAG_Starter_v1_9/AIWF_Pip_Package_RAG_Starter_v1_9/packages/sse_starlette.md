...
## Identity
- **pip name:** `sse-starlette`
- **import/module clue:** `sse_starlette`
- **category:** `server_sent_events_streaming`
- **AIWF confidence:** source-reviewed for v1.6 LLM serving and OpenAI-compatible local server support.

## What it is
Server-Sent Events helper package used for streaming token/event responses in Starlette/FastAPI-style services.
...
```powershell
python -m pip show sse-starlette
python - <<'PY'
import importlib
mod = 'sse_starlette'
try:
...
## Sources
https://github.com/sysid/sse-starlette | https://pypi.org/project/sse-starlette/
