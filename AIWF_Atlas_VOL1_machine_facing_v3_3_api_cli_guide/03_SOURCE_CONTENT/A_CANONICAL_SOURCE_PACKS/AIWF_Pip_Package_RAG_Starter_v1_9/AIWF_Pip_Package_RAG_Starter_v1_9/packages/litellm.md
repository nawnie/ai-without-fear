...
## Identity
- **pip name:** `litellm`
- **import/module clue:** `litellm`
- **category:** `llm_gateway_openai_router`
- **AIWF confidence:** source-reviewed for v1.6 LLM serving and OpenAI-compatible local server support.

## What it is
OpenAI-format LLM gateway/proxy/router for local and remote model providers; useful for swapping vLLM, Ollama, OpenAI, and other endpoints behind one interface.
