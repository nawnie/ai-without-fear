...
## What it is
cuDNN backend runtime libraries for CUDA 13-family Python environments.
...
Use only for stacks that explicitly support CUDA 13 component wheels. Keep isolated from stable CUDA 12 ComfyUI environments unless the framework/custom node support matrix says otherwise.
...
1. `python -m pip show nvidia-cudnn-cu13` if this is an installed package path.
...
## AIWF install stance
Treat CUDA 13 cuDNN as a future/experimental lane until the target framework and extension stack explicitly support it. Do not mix cu12 and cu13 cuDNN packages inside the same support wheelhouse without a manifest reason.
