...
## RAG purpose
Nomic Atlas/API client for hosted embeddings and dataset workflows.
...
Requires account/API context for hosted features.
...
```powershell
python -m pip show nomic
python -c "import importlib; m=importlib.import_module('nomic'); print('nomic import ok', getattr(m, '__version__', 'version not exposed'))"
```
...
## RAG tags
`pip-package`, `v0.9-custom-node-training-web-deps`, `low`, `low`, `windows-ai`, `embeddings`

## Source URLs
- https://pypi.org/project/nomic/
- https://docs.nomic.ai/atlas/python-api/generate-embeddings
