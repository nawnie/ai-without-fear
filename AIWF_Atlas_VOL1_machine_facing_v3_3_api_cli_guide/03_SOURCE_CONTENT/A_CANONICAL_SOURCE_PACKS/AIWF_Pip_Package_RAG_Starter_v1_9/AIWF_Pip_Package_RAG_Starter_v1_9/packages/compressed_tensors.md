...
## Identity
- **pip name:** `compressed-tensors`
- **import/module clue:** `compressed_tensors`
- **category:** `llm_quantization_format`
- **AIWF confidence:** source-reviewed for v1.6 LLM serving and OpenAI-compatible local server support.

## What it is
Model compression/quantization format package used around deployable compressed model artifacts.
...
```powershell
python -m pip show compressed-tensors
python - <<'PY'
import importlib
