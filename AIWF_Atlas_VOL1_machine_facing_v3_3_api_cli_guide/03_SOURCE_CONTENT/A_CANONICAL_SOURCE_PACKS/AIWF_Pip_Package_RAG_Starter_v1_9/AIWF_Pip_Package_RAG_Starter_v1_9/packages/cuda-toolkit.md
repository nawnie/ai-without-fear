...
## Identity
- **pip name:** `cuda-toolkit`
- **import/module clue:** `cuda_toolkit`
- **category:** `nvidia_cuda_toolkit_metapackage`
- **AIWF confidence:** source-reviewed for role, install-risk classification, and cuDNN/CUDA stack placement.

## What it is
PyPI CUDA Toolkit metapackage path for installing optional CUDA Toolkit components into Python environments.
...
Use carefully: the CUDA Toolkit is a developer toolchain/runtime family, not the same thing as the NVIDIA display driver and not the same thing as cuDNN.
...
1. `python -m pip show cuda-toolkit` if this is an installed package path.
...
## AIWF install stance
For source builds, system CUDA Toolkit plus MSVC is still the clearest Windows path. PyPI toolkit components can help dependency resolution, but the support manifest must say whether the build used system Toolkit, pip Toolkit, or framework-bundled libraries.
