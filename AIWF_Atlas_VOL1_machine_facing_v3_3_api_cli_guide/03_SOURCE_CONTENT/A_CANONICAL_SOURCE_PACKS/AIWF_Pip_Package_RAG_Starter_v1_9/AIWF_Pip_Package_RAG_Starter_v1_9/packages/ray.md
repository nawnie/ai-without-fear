...
## Identity
- **pip name:** `ray`
- **import/module clue:** `ray`
- **category:** `distributed_serving_runtime`
- **AIWF confidence:** source-reviewed for v1.6 LLM serving and OpenAI-compatible local server support.

## What it is
Distributed runtime used by Ray Serve LLM and some vLLM scale-out paths; not required for simple one-GPU local serving.
