...
## Identity
- **pip name:** `nvidia-cuda-nvcc-cu12`
- **import/module clue:** `nvidia.cuda_nvcc`
- **category:** `nvidia_cuda_compiler`
- **AIWF confidence:** source-reviewed for role and install-risk classification.

## What it is
NVIDIA CUDA compiler package in the CUDA 12 pip component family; used when pip-based toolchains expose nvcc components.
...
Still validate cl.exe/MSVC and CUDA_HOME; nvcc alone does not guarantee a usable Windows build chain.
...
1. Confirm the package is actually installed in the active venv: `python -m pip show nvidia-cuda-nvcc-cu12`.
