...
## Identity
- **pip name:** `cuda-core`
- **import/module clue:** `cuda.core`
- **category:** `nvidia_cuda_python_core`
- **AIWF confidence:** source-reviewed for role, install-risk classification, and cuDNN/CUDA stack placement.

## What it is
Pythonic access layer for CUDA Runtime and core CUDA functionality in the NVIDIA CUDA Python ecosystem.
...
May be used by modern diagnostic/tooling stacks that want Python-level CUDA access without writing C++.
...
1. `python -m pip show cuda-core` if this is an installed package path.
...
## AIWF install stance
Support useful, but do not confuse with torch.cuda. Record it as a diagnostic/developer layer.
