...
## Identity
- **pip name:** `nvidia-cuda-runtime-cu13`
- **import/module clue:** `nvidia.cuda_runtime`
- **category:** `nvidia_cuda_runtime`
- **AIWF confidence:** source-reviewed for role and install-risk classification.

## What it is
CUDA runtime libraries for CUDA 13-family Python environments where available; runtime only, not full Toolkit.
...
Treat CUDA 13 wheels as bleeding-edge; verify PyTorch/custom-node support before mixing into ComfyUI.
...
1. Confirm the package is actually installed in the active venv: `python -m pip show nvidia-cuda-runtime-cu13`.
