...
## Identity
- **pip name:** `nvidia-cudnn-jit-cu12`
...
## What it is
NVIDIA cuDNN JIT package lane for CUDA 12-family component installs.
...
Track separately from nvidia-cudnn-cu12 because JIT/runtime-compiled cuDNN engine behavior can have different DLL and version implications.
...
1. `python -m pip show nvidia-cudnn-jit-cu12` if this is an installed package path.
...
## AIWF install stance
Use only when a project explicitly asks for cuDNN JIT components. Support triage should capture this package because it changes the likely DLL/engine loading story.
