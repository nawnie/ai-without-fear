...
## Identity
- **pip name:** `cuda-pathfinder`
- **import/module clue:** `cuda.pathfinder`
- **category:** `nvidia_cuda_component_discovery`
- **AIWF confidence:** source-reviewed for role, install-risk classification, and cuDNN/CUDA stack placement.

## What it is
NVIDIA CUDA Python utility layer for locating CUDA components installed in the Python environment.
...
Important for AIWF support tooling because it can help explain which CUDA/cuDNN component path a Python environment is seeing.
...
1. `python -m pip show cuda-pathfinder` if this is an installed package path.
...
## AIWF install stance
Good candidate for future AIWF environment validator integration: identify CUDA component visibility before guessing PATH fixes.
