...
## Identity
- **pip name:** `pytriton`
- **import/module clue:** `pytriton`
- **category:** `triton_python_serving_wrapper`
- **AIWF confidence:** source-reviewed for Triton serving/deployment support placement.

## What it is
Pythonic wrapper around Triton serving concepts for binding Python callables/models into Triton-backed local inference services.
...
Treat as a serving/app package, not a ComfyUI dependency. Verify whether the workflow needs embedded PyTriton versus full Triton server model repository deployment.
...
1. `python -m pip show pytriton`.
2. Verify import/module where applicable: `pytriton`.
...
## Sources
https://triton-inference-server.github.io/pytriton/ | https://docs.nvidia.com/deeplearning/triton-inference-server/
