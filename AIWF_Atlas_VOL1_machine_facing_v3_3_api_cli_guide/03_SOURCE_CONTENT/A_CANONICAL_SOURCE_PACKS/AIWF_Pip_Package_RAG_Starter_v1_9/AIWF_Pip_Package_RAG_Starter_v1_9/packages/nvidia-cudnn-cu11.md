...
## Identity
- **pip name:** `nvidia-cudnn-cu11`
- **import/module clue:** `nvidia.cudnn / DLL package`
- **category:** `nvidia_cudnn_backend_runtime_legacy`
- **AIWF confidence:** source-reviewed for role, install-risk classification, and cuDNN/CUDA stack placement.

## What it is
Legacy CUDA 11-family cuDNN backend runtime package path visible in NVIDIA package indexes.
...
Keep for older model tooling, older ONNX Runtime/PyTorch lanes, and archive support. Do not install into modern CUDA 12/13 stacks unless intentionally reproducing an older environment.
...
1. `python -m pip show nvidia-cudnn-cu11` if this is an installed package path.
...
## AIWF install stance
This is a legacy-lane package. The support system should suggest it only when environment evidence points to CUDA 11.x/cuDNN 8-era compatibility.
