...
9. **RAG directories need a Source Ledger first.**
   - Every RAG topic lane starts with `00_SOURCE_LEDGER.md`.
   - Every source gets a quality grade and AIWF decision label.
   - Retrieval cards should separate tested claims from open questions.
   - Use `RAG_DIRECTORY_ARCHITECTURE.md` as the shared standard.
...
| RAG Source Ledger | inside Field Manual first, later Knowledge Pack | New standard | High |
...
### Phase 4.5 — RAG Source Ledger Standard
...
- four RAG topic lanes
- source ledger template
- source quality scale
- AIWF decision labels
- retrieval-ready card template
- open question tracking
- JSONL / Markdown export preparation

Rule:

Do not let RAG become a pile of unsorted Markdown. Every source should have a decision: use, wrap, document, build the missing piece, watch, or avoid for now.
...
### Phase 4.6 — Runtime Package Ledger

Initial location:

```text
AI-Field-Manual-2026/rag/02_python_pip_and_runtime_packages/
```

Purpose:

- convert package research into source-backed install decisions
- support one-click installers without creating dependency hell
- guide EnvPack diagnostics for torch, CUDA, ONNX Runtime, OpenCV, Gradio, Diffusers, Transformers, SAM2, Grounding DINO, and restoration stacks
- define the venv-island/profile-runner architecture for future AIWF Gradio apps

Do not create a single global requirements file for all AIWF tools. Use profile-specific requirements and PowerShell setup commands.

### Phase 4.6 — ML Theory / Training RAG Lane

Initial location:

```text
AI-Field-Manual-2026/rag/03_ml_theory_training_and_model_behavior/
```

Scope:

- beginner-safe theory explanations
- training ladder from prompting to fine-tuning
- diffusion/latent/token/attention/LoRA/ControlNet concept cards
- Volume I vs Volume II boundaries
- retrieval exports for future tooltips and local helper assistants

Rule:

> Volume I teaches mechanisms as operator intuition. Volume II stores math, architecture, optimization, evaluation, and post-training depth.

## v0.6 Added Lane Focus — Editing Operator Theory

The ML theory RAG lane now has a direct bridge into workflow building:

- teach CFG as prompt pressure;
- teach denoise as source rewrite permission;
- teach masks as edit contracts;
- connect inpaint failures to mask, denoise, prompt, and control/reference inputs;
- route workflow QA requirements back into Workflow Packs, ComfyUI Nodes, and EnvPack.

This should support SDXL inpaint, old-photo restoration, face-safe edits, crop-and-stitch repair, and future Gradio modules.
...
### Unique retained material from `CONTENT/AIWF-Project-Chat-Distribution-Roadmap-v0.5-comfyui-core-api-subgraph/AIWF-Project-Chat-Distribution-Roadmap/MASTER_DISTRIBUTION_ROADMAP.md`
...
| AIWF Photos | future app repo | Concept | Later |
| Labs / Preservation Bridge | `nawnie/AIWF-Labs` | Experimental | Later |
### Phase 5.5 — ComfyUI Schema / Manifest Glue
- capture `/object_info` from the user's running ComfyUI
- validate workflow class types and input sockets
- generate workflow manifests
- separate local nodes from API/partner nodes
- support Subgraph/App Mode packaging rules
- design branch outputs for Partial Execution
This phase supports EnvPack, Workflow Packs, and ComfyUI Nodes. It should not become a separate product until the beta repos are stable.
