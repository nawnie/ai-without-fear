# Facilitator Slide Deck Outline

This outline supports one master deck with six sections. Each session may be exported as a separate deck.

## Opening slides

1. AI Without Fear - Community AI Readiness Lab
2. Program promise: understand, use, govern
3. AI as a force multiplier, not a substitute for judgment
4. Training safety rule: fictional, public, or sanitized data only
5. The six-session path
6. What every organization will leave with

## Session 1 - AI Without the Fog Machine

7. Where AI is already hiding in ordinary tools
8. Input -> model/tools -> output -> human review
9. Fluent is not the same as factual
10. Capability does not equal permission
11. Useful now / needs safeguards / do not use yet
12. Activity instructions
13. Debrief: what remains a human responsibility?

## Session 2 - The Data Traffic Light

14. Why data classification comes before prompting
15. Green data
16. Yellow data
17. Red data
18. Harmless pieces can combine into sensitive information
19. Uncertain means pause and escalate
20. Classification activity
21. Organization data map

## Session 3 - Prompt, Review, Verify

22. Prompting is task specification
23. Task, context, audience, constraints
24. Allowed source material and output format
25. Weak prompt versus reviewable prompt
26. Verification ladder
27. Names, dates, numbers, quotations, links
28. One-change rule
29. Guided prompt lab

## Session 4 - Tool and Vendor Check

30. The tool inventory problem
31. Account ownership and recovery
32. Data retention and model training questions
33. Exact plan and settings matter
34. Actions, connectors, and blast radius
35. Export, deletion, cost, and fallback
36. Tool decision categories
37. Review worksheet activity

## Session 5 - The Policy Sprint

38. A policy is a decision map
39. Approved, restricted, prohibited
40. Human review and final accountability
41. Disclosure rules
42. Tool owner and incident contact
43. Minimum viable one-page policy
44. Scenario test

## Session 6 - One Safe Workflow

45. Choose low-impact, reversible, reviewable work
46. Generate -> Select -> Refine -> Finish
47. Workflow documentation fields
48. Final commitment: what changes next week?

## Visual production notes

- Use the AIWF blue-and-white field-manual visual language as the base.
- Add one diagram per concept, not decorative stock imagery.
- Use Torchie for short reassurance or troubleshooting callouts, not as the authority on safety or policy.
- Use the page-6 ControlNet diagrams from the source manual only after asset rights and attribution are confirmed. They are useful examples of conditioning, but they are not needed in the community core deck.
- Build new original diagrams for the Data Traffic Light, verification ladder, policy loop, and workflow loop.
